...
\begin{column}{0.4\textwidth}
      ...
      \begin{center}
        \begin{tikzpicture}[x=0.52cm,y=0.52cm]
          ...
          \draw[->] (0,0) -- (0,4.9) node[above] {value};
          % Same vertical scale factor is used for both functions.
          % Here the displayed y-value equals the true value divided by 2.
          \draw[very thick,ForestGreen,domain=0.4:5.9,samples=120]
            plot (\x,{\x}) node[right] {$f(n)=2n$};
          \draw[very thick,orange!85!black,domain=0.4:5.9,samples=120]
            plot (\x,{(\x*\x)/4}) node[above right] {$g(n)=\frac12 n^2$};
          \draw[dashed,thick] (4,0) -- (4,4);
          \node[below] at (4,0) {$n=4$};
        \end{tikzpicture}
